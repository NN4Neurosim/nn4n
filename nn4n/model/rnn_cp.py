...
class RNN_CP(BaseNN):
    ...
    def _handle_warnings(self, kwargs):
        ...
        if 'sparsity_masks' in kwargs:
            warnings.warn("sparsity_masks is deprecated. Use `sparsity_masks` instead.",
                          UserWarning, stacklevel=slevel)
        if 'ei_masks' in kwargs:
            warnings.warn("ei_masks is deprecated. Use `ei_masks` instead.",
                          UserWarning, stacklevel=slevel)
        if 'plasticity_masks' in kwargs:
            warnings.warn("plasticity_masks is deprecated. Use `plasticity_masks` instead.",
                          UserWarning, stacklevel=slevel)
        for key in kwargs:
    ...
    def _check_masks(self, param, param_type, dims):
        ...
        if param is None:
            if param_type in ["ei_masks", "sparsity_masks", "plasticity_masks", "biases"]:
                ...
                raise ValueError(f"{param_type} cannot be None when param_type is {
                                 param_type}")  # weights
    ...
    def _check_parameters(self):
        ...
        for i in self.dims:
            assert type(i) == int, f"dims must be a list of integers, {
                i} is not an integer"
