...
class MultiIO(BaseMask):
    def __init__(self, **kwargs):
        ...
        self.input_table = kwargs.get("input_table", np.ones(
            (self.n_input_groups, self.dims[1])))
        self.readout_table = kwargs.get("readout_table", np.ones(
            (self.n_readout_groups, self.dims[1])))
    ...
    def _check_parameters(self):
        ...
        assert type(self.input_dims) == list and self._check_int_list(
            self.input_dims), "input_dims must be a list of integers"
        assert type(self.readout_dims) == list and self._check_int_list(
            self.readout_dims), "readout_dims must be a list of integers"

        # Check if the input_dims and readout_dims all sum up to self.dims[0] and self.dims[2]
        assert np.sum(
            self.input_dims) == self.dims[0], "input_dims must sum-up to the full input dimension specified in self.dims[0]"
        assert np.sum(
            self.readout_dims) == self.dims[2], "readout_dims must sum-up to the full readout dimension specified in self.dims[2]"

        # Check if the input/readout table dimension is valid
        assert self.input_table.shape == (self.n_input_groups, self.dims[1])
        assert self.readout_table.shape == (
            self.n_readout_groups, self.dims[1])
    ...
    def _generate_readout_mask(self):
        ...
        for i, d in enumerate(self.readout_dims):
            ...
            readout_mask[:, dim_counter:dim_counter +
                         d] = np.tile(readout_idx, d)
            dim_counter += d
