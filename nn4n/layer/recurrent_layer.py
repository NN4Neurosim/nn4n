...
class RecurrentLayer(nn.Module):
    ...
    def __init__(self, layer_struct, **kwargs):
        ...
        _alpha = torch.full((self.hidden_size,),
                            layer_struct['dt']/layer_struct['tau'])
        self.alpha = torch.nn.Parameter(
            _alpha, requires_grad=True) if layer_struct['learn_alpha'] else _alpha

        self.input_layer = LinearLayer.from_dict(layer_struct['in_struct'])
        self.hidden_layer = HiddenLayer(
            layer_struct=layer_struct['hid_struct'])
    ...
    def _recurrence(self, fr_t, v_t, u_t):
        ...
        if self.preact_noise > 0:
            preact_epsilon = torch.randn(
                (u_t.size(0), self.hidden_size), device=u_t.device) * self.preact_noise
        ...
        if self.postact_noise > 0:
            postact_epsilon = torch.randn(
                (u_t.size(0), self.hidden_size), device=u_t.device) * self.postact_noise
