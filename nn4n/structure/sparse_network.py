...
class SparseNetwork(BaseStuct):
    def __init__(self, **kwargs):
        """
        Generate a network with sparse input
        ...
        if self.__class__.__name__ == "SparseNetwork":
    ...
    def _generate_readout_mask(self):
        ...
        self.readout_mask[np.ix_(np.arange(self.output_dim), self.readout_idx)] = self._generate_sparse_matrix(self.output_dim, len(self.readout_idx), 1)
